        ...
        <el-dialog v-model="detailDialogVisible" title="情绪日志详情" width="800px" :close-on-click-modal="false">
            <div class="detail-content" v-if="currentDetail">
                ...
                <div class="detail-section">
                    ...
                    <el-button
                        v-if="currentDetail.contentMasked"
                        style="margin-top: 12px"
                        type="primary"
                        plain
                        @click="revealFullContent"
                    >查看完整原文</el-button>
                </div>
                <div class="detail-section">
                    <h4>AI情绪分析结果</h4>
            ...
            <template #footer>
                <el-button v-if="currentDetail?.contentMasked" type="primary" plain @click="revealFullContent">查看完整原文</el-button>
...
import { getEmotionalPage, getEmotionalDetail, deleteEmotional } from '@/api/admin'
...
const revealFullContent = () => {
    ElMessageBox.confirm('将显示用户写下的原文，请只在必要的帮助场景查看。', '查看原文', {
        confirmButtonText: '我需要查看',
        cancelButtonText: '先不看',
        type: 'warning',
    }).then(() => getEmotionalDetail(currentDetail.value.id, { reveal: true })).then((data) => {
        currentDetail.value = data
        if (data.aiEmotionAnalysis) {
            aiData.value = JSON.parse(data.aiEmotionAnalysis)
        }
    })
}
