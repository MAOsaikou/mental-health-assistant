<template>
    <div>
        ...
        >
            ...
            <template #footer>
                <el-button v-if="messagesMasked" type="primary" plain @click="revealMessages">查看完整对话</el-button>
...
const messagesMasked = computed(() => sessionMessages.value.some((item) => item.contentMasked))

const loadMessages = (sessionId, reveal = false) => {
    loadingMessages.value = true
    getSessionDetail(sessionId, reveal ? { reveal: true } : undefined).then((res) => {
...
const viewSessionDetail = (row) => {
    ...
    loadMessages(row.id, false)
}

const revealMessages = () => {
    ElMessageBox.confirm('将显示用户完整对话原文，请只在必要的帮助场景查看。', '查看完整对话', {
        confirmButtonText: '我需要查看',
        cancelButtonText: '先不看',
        type: 'warning',
    }).then(() => loadMessages(currentSessionId.value, true))
}
